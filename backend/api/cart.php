...
switch ($_SERVER['REQUEST_METHOD']) {
    ...
    case 'POST':
        if (empty($body['food_id']))
            sendResponse(['success' => false, 'message' => 'Food ID is required.'], 422);
    ...
    case 'PUT':
        if (!$id)
            sendResponse(['success' => false, 'message' => 'Cart item ID required.'], 422);
        if (!isset($body['quantity']))
            sendResponse(['success' => false, 'message' => 'Quantity is required.'], 422);
